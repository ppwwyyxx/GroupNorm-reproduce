...
class GoogleNetResize(imgaug.ImageAugmentor):
    ...
    def __init__(self, crop_area_fraction=0.08,
                 aspect_ratio_low=0.75, aspect_ratio_high=4.0 / 3.0,
                 target_shape=224):
        self._init(locals())
    ...
    def _augment(self, img, _):
        ...
        for _ in range(10):
            ...
            if hh <= h and ww <= w:
                ...
                out = cv2.resize(out, (self.target_shape, self.target_shape), interpolation=cv2.INTER_LINEAR)
                return out
        out = imgaug.ResizeShortestEdge(self.target_shape, interp=cv2.INTER_LINEAR).augment(img)
